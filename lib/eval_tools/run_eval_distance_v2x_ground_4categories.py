...
def evaluate_SurveillanceCamera(eval_range_min=0, eval_range_max=120, eval_range_step=30, logger='', 
                                set_type=['vehicle', 'car', 'van', 'bus', 'truck'],
                                coarse_label_path=None, iou_thresh=0.5,result_path='',calib_dir='',de_norm_dir=''):
    ...
    last_folder_name = result_path.split("/")[-1]
    detail_path = result_path.replace(last_folder_name, last_folder_name + "_detail")

    os.system('mkdir -p %s' % (detail_path))

    detail_file = '%s/detail.txt' % (detail_path)
    folder_seq_res = "%s/" % (result_path)
    folder_seq_gt = '%s/' % (coarse_label_path)
    ...
    for i, name in enumerate(list_seq):
        ...
        anno_this = None
        intrinsic = read_kitti_cal(os.path.join(calib_dir, name_key + '.txt'))
        intrinsic = numpy.matrix(intrinsic).reshape(4, 4)

        denorm = read_kitti_denorm(os.path.join(de_norm_dir, name_key + '.txt'))  
    ...
    plt.savefig('%s/result_img.png' % (detail_path))
    # plt.show()
