"""
KITTI Dataset Loader for MonoUNI
Adapted from rope3d.py to support KITTI format data (converted from ROPE3D)
"""
import os
import numpy as np
import torch
...
class KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        self.class_name = self.writelist  # class list from config
        self.num_classes = len(self.class_name)
        self.cls2id = {cls_name: i for i, cls_name in enumerate(self.class_name)}  # build class-to-id map
        self.max_objs = 50
        self.resolution = np.array([960, 512])  # W * H
        self.use_3d_center = cfg['use_3d_center']
        # KITTI format: Van->Car, Truck->Bus already converted
        # DontCare is filtered out in rope2kitti.py
        ...
        self.denorm_dir = os.path.join(self.data_dir, 'training', 'denorm')
        self.box3d_dense_depth_dir = os.path.join(self.data_dir, 'training', 'box3d_depth_dense')
